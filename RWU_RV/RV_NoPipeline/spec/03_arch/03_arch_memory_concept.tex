...
Each cache uses two X-Fab SRAM macros: one tag SRAM and one data SRAM.
Both macros operate as single-port synchronous SRAMs.

\begin{table}[H]
\caption{SRAM Macro Requirements per Cache}
...
\begin{tabularx}{\textwidth}{|l |l |l |l |X|}
  ...
  \hline
  \hline
  Tag SRAM (I) & 96\,bit & 32 & 384\,B & Holds valid + tag for all 4 ways + 3 PLRU bits; rounded from 95 \\
  \hline
  Tag SRAM (D) & 96\,bit  & 32 & 384\,B & Holds valid + tag for all 4 ways + 3 PLRU bits; rounded from 95 (no dirty bit) \\
  \hline
  Data SRAM (I) & 256\,bit & 128 & 4\,KB & 32 sets $\times$ 4 ways $\times$ 32\,B/line; addressed as \{index,way\} \\
  \hline
...
\textbf{Note:} The specific X-Fab SRAM macro type (cut), including port
configurations and timing parameters, is to be selected from the X-Fab
SRAM compiler catalogue and will be documented in a separate SRAM
specification.
...
\begin{tabularx}{\textwidth}{|l |X |}
  ...
  \hline
  \hline
  \texttt{IDLE}      & Waiting for a fetch request from the CPU \\
  \hline
...
\begin{itemize}
  \item \textbf{Flash region (cacheable, read-only):} the request goes
    ...
    asserts an error signal (behaviour TBD, see MEM-06).
...
\begin{tabularx}{\textwidth}{|l |X |}
  ...
  \hline
  \hline
  \texttt{IDLE}       & Waiting for a load/store request from the CPU \\
  \hline
...
\begin{tabularx}{\textwidth}{|l |l |l |X|}
  ...
  \hline
  \texttt{ic\_stall\_o} & in  & 1  & High while a cache miss is being serviced; CPU must freeze PC \\
  \hline
...
interface as_icache_if (input logic clk_i, 
  ...
  logic [31:0] ic_rdata;
  logic        ic_rvalid;
  modport cpu  (output ic_addr, ic_req,  
                input  ic_stall, ic_rdata, ic_rvalid);
  modport cache(input  ic_addr, ic_req,  
                output ic_stall, ic_rdata, ic_rvalid);
endinterface
\end{verbatim}
...
\begin{tabularx}{\textwidth}{|l |l |l |X|}
  ...
  \hline
  \texttt{dc\_stall\_o} & in  & 1  & High while cache miss or write-back is in progress \\
  \hline
...
interface as_dcache_if (input logic clk_i, 
  ...
  modport cpu  (output dc_addr, dc_req, dc_wr, dc_size, 
                       dc_wdata, dc_wstrb,
                input  dc_stall, dc_rdata, dc_rvalid);
  modport cache(input  dc_addr, dc_req, dc_wr, dc_size, 
                       dc_wdata, dc_wstrb,
                output dc_stall, dc_rdata, dc_rvalid);
endinterface
\end{verbatim}
...
\begin{tabularx}{\textwidth}{|l |X |l|}
  ...
  \hline
  MEM-02 & X-Fab SRAM macro selection: choose specific cut from SRAM compiler
           catalogue (single-port, width/depth match); document timing
           derating and hold margins. & TBD \\
  \hline
  MEM-03 & Cache flush / invalidate mechanism for software (CSR registers or
           fence.i support): required before mode switches and DMA transfers. & TBD \\
  \hline
  MEM-04 & D-Cache coherency with future DMA engine: define invalidation
           protocol. & TBD \\
  \hline
  ...
  \hline
  MEM-06 & AXI4 error handling: define CPU-visible trap or interrupt behaviour
           on RRESP/BRESP\,$\neq$\,OKAY. & TBD \\
  \hline
  MEM-07 & QSPI XIP mode vs.\ cache-miss-driven burst: clarify whether the
           QSPI controller operates in Execute-In-Place (XIP) mode and the
           cache controller issues normal read bursts, or whether a different
           command sequence is required per miss. & TBD \\
  \hline
\end{tabularx}
\end{table}
