...
\begin{tabularx}{\textwidth}{|l |l |l |X|}
  ...
  \hline
  \texttt{ic\_stall\_o}      & in  & 1  & High while a cache miss or flush is in progress; CPU must freeze PC \\
  \hline
...
interface as_icache_if (input logic clk_i,
  ...
  modport cpu  (output ic_addr, ic_req, ic_flush,
                input  ic_stall, ic_flush_done, ic_rdata, ic_rvalid);
  modport cache(input  ic_addr, ic_req, ic_flush,
                output ic_stall, ic_flush_done, ic_rdata, ic_rvalid);
endinterface
\end{verbatim}
...
\begin{tabularx}{\textwidth}{|l |l |l |X|}
  ...
  \hline
  \texttt{dc\_stall\_o}      & in  & 1  & High while cache miss, scratchpad access, or flush is in progress \\
  \hline
...
interface as_dcache_if (input logic clk_i,
  ...
  modport cpu  (output dc_addr, dc_req, dc_wr, dc_size,
                ...
                input  dc_stall, dc_flush_done, dc_rdata, dc_rvalid);
  modport cache(input  dc_addr, dc_req, dc_wr, dc_size,
                       dc_wdata, dc_wstrb, dc_flush,
                output dc_stall, dc_flush_done, dc_rdata, dc_rvalid);
endinterface
\end{verbatim}
