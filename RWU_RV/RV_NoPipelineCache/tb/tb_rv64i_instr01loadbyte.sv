  ...
  always @(negedge clk_core_s) begin
    if(cs_s === 1) begin
      ...
      case(gpio_s)
          ...
          128     : begin $display("Instr 01 - load byte08: 0x%0h", gpio_s);
                          $display("Simulation load byte succeeded"); #100; #(1*2*clk_2_t);
                          $fdisplay(fd,"%s - load byte: Test ok", get_time());
                          $fclose(fd); $stop; end
          default : begin $display("Unexpected GPIO: 0x%0h", gpio_s);
                          $fdisplay(fd,"%s - load byte: Test fail", get_time());
                          $fclose(fd); $stop; end
      endcase
    end
  end
