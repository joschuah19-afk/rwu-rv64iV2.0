  ...
  always @(negedge clk_core_s) begin
    if(cs_s === 1) begin
      ...
      case(gpio_s)
            ...
            1     : begin $display("Instr 07 - slli: 0x%0h", gpio_s);
                          $display("Simulation slli succeeded"); #100; #(1*2*clk_2_t);
                          $fdisplay(fd,"%s - slli: Test ok", get_time());
                          $fclose(fd); $stop; end
          default : begin $display("Unexpected GPIO: 0x%0h", gpio_s);
                          $fdisplay(fd,"%s - slli: Test fail", get_time());
                          $fclose(fd); $stop; end
      endcase
    end
  end
