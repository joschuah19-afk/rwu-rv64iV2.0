...
module tb_rv64i ();
  ...
  always @(negedge clk_core_s) begin
    if(cs_s === 1) begin
      $display("CS detected");
      case(gpio_s)
           29      : begin $display("Instr 29 - lui, addi, sb: 0x%0h", gpio_s);
                          $display("Simulation lui succeeded"); #100; #(1*2*clk_2_t);
                          $fdisplay(fd,"%s - lui: Test ok", get_time());
                          $fclose(fd); $stop; end
          default : begin $display("Unexpected GPIO: 0x%0h", gpio_s);
                          $fdisplay(fd,"%s - lui: Test fail", get_time());
                          $fclose(fd); $stop; end
      endcase
    end
  end
